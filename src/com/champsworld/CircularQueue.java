...
public final class CircularQueue<T>{
    ...
    private static class QState {
        ...
        void reset(QState state){
            ...
            if(max_size < state.max)
              this.data = new Object[state.max];
            
            System.arrayCopy(state.data, 0, this.data,0, state.max);
            
            this.produceIdx = state.produceIdx;
            this.consumeIdx = state.consumeIdx;
            this.max_size = state.max;
        }
        ...
        // produce idx is supposed to be always larger then consumeIdx
    ...
    //if we use AtomicInteger we have to use getAndIncrment which will produce error when queue is full, our counter is overshot
    ...
    public CircularQueue(int initial_capacity){
        this.currentCapacity = new AtomicInteger(initial_capacity);
        this.qState = new AtomicStampedReference<>(new QState(initial_capacity), stampGenerator.getAndIncrement());
        cachedSwapState =ThreadLocal.withInitial(()->new QState(this.qState.get().max_size));
    }
    
    // queues item at the end of the queue
    public boolean add(T value){
        if(value==null) return new IllegalArgumentException("");
        ...
        while(true) {
            ...
            if (reference.isFull()) {                 
                ...
                newReference = CACHED_TRIPLET.get();
                reference = this.triplet.get(stampHolder);
            }            
    ...
    private boolean increaseSize() {
        QState reference;
        final int[] stampHolder =new int[1];
        final int stamp = stampGenerator.getAndIncrement();
        QState newReference ;
        ...
        do {
            ...
            if(consIdx!=0) {
                ...
                System.arraycopy(reference.data, consIdx, data_swap, 0 , curSize - consIdx);
                System.arraycopy(reference.data, 0, data_swap, curSize -consIdx, consIdx);
            } else {
                System.arraycopy(reference.data, 0, data_swap, 0 , currSize);
            }
            if(newReference ==null)
              newReference = new Triplet(data_swap, newSize, currSize, 0);
            else newReference.reset(data_swap, newSize, currSize, 0);
            prevNewSize = newSize;
    ...
    public int size() {
        return this.qState.get().size();        
    }
    
    public boolean isEmpty(){
        return this.qState.get().isEmpty();
    }
    
    
    public boolean isFull() {
        return this.qState.get().isFull();
    }    
